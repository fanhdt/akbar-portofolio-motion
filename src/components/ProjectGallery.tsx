      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-12">
        ...
        {filteredProjects.map((project) => (
          <Link key={project.slug} href={`/project/${project.slug}`} className="group block">
            <ProjectVideoThumb title={project.title} videoUrl={project.videoUrl} /> {/* Judul Karya di Bawah Video */}{" "}
            <h3 className="text-sm font-medium text-neutral-900 group-hover:text-neutral-500 transition-colors">{project.title}</h3>{" "}
          </Link>
        ))}{" "}
      </div>
      {/* State jika kategori kosong */} {filteredProjects.length === 0 && <div className="w-full text-center py-20 text-neutral-500 text-sm">Belum ada karya di kategori ini.</div>}{" "}
    </section>
  );
}
